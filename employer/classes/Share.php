...
class Share extends Db
{
    ...
    public function sharedBy($orgId)
    {
    ...
    public function sharedWith($orgId)
    {
        $sql = "SELECT * FROM shared_doc 
        JOIN users ON shared_doc.shared_with =users.idusers
        JOIN document ON shared_doc.shared_do_id=document.iddocument   
        WHERE users.user_org = ? ORDER BY shared_date DESC LIMIT 5";
        $stmt = $this->dbconn->prepare($sql);
        $stmt->execute([$orgId]);
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function sharedDocs($orgId)
    {
        $sql = "SELECT 
            ...
            WHERE user_shared_by.user_org = ? OR user_shared_with.user_org = ?";
    ...
    public function shareDoc($doc_title, $doc_desc, $doc_file, $doc_creator, $sender_team, $email, $username, $coll_desc)
    {
        ...
        try {
            ...
            $sql2 = "INSERT INTO document (doc_title, doc_desc, document_file, doc_creator, creator_team) VALUES (?, ?, ?, ?, ?)";
            $stmt2 = $this->dbconn->prepare($sql2);
            $result2 = $stmt2->execute([$doc_title, $doc_desc, $doc_file, $doc_creator, $sender_team]);
