...
class Document extends Db
{
    ...
    public function getRecentDocuments($userId)
    {
        try {
            ...
            JOIN tagmapping ON tagmapping.doc_id = document.iddocument
            JOIN doctag ON tagmapping.doc_id = doctag.iddoctag
            WHERE users.user_org = ? ORDER BY upload_date DESC LIMIT 5";
            $stmt = $this->dbconn->prepare($sql);
            $stmt->execute([$userId]);
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $result ? $result : array(); // Return the result if it's not empty, otherwise return an empty array
            ...
            if ($result) {
                ...
                if ($tagResult) {
                    ...
                    if ($mapResult) {
                        ...
                        // update doc table
                        // if($mapResult){
                        //     $mapId = $this->dbconn->lastInsertId();
                        //     $docSLQ = "";
                        // }
