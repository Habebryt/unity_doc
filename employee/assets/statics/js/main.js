...
$('#viewDoc').on('show.bs.modal', function (e) {

  //get data-id attribute of the clicked element
  var docId = $(e.relatedTarget).data('user');
  alert(docId);
  //make an ajax call to receive an array based on userid, that is you can now pass the userid to an ajax that will help you fetch the details you need. let's say the ajax returns t ,     

  $(e.currentTarget).find('.modal-body-doc-body').html(t); //place t on any location: lets display t inside the div with class modal-body
  $(e.currentTarget).find('.modal-title').html("Details for User" + docId); //displays this in the div with class modal-title

});

// Get the form and modal elements
const viewDocForm = document.getElementById('viewDocForm');
const viewDocModal = document.getElementById('viewDoc');
...
viewDocModal.addEventListener('show.bs.modal', function (event) {
  ...
  const docId = viewDocForm.querySelector('input[name="docid"]').value;

  // Populate the modal body with the document ID
